...
if ($first == 'import_rumble') {

   $user_id = $_GET['user_id'];
   $rumble = $_GET['rumble'];
   
   try{
      $html = file_get_contents( $rumble );
      ...
      $description =getCurlDescription($rumble);
      ...
      $tags = preg_replace("/[^a-zA-Z\s]/", "", $titulo[1]);
       ...
       if ($db->where('rumble', $video_import_id)->getValue(T_VIDEOS, 'count(*)') > 0) {
       ...
       $videos = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE title LIKE '%$title%'");
       if (!empty($video)){
          if (count($videos) > 0) {
             $data = array('status' => 400, 'message' => $error_icon . $lang->video_already_exist);
             header('Content-Type: application/json');
       ...
       $data_insert      = array(
        ...
        $data_insert['rumble'] = $video_import_id;
       ...
       if ($insert) {
            ...
            $data = array(
              'status' => 200,
              'message' => $success_icon . $title . ' - ' . $lang->successfully_uplaoded
            );
       }else{
           $data = array(
              'status' => 200,
              'message' => $error_icon . $db->getLastError()
           );
       }
       echo json_encode($data);
...
if ($first == 'import_youtube') {
   ...
   try {
    ...
    if ($db->where('youtube', $youtube_id)->getValue(T_VIDEOS, 'count(*)') > 0) {
        $data = array('status' => 400, 'message' => $error_icon . $lang->video_already_exist);
        header('Content-Type: application/json');
        echo json_encode($data);
        exit();
    ...
    if (!empty($video)){
     ...
     $youtube = new  Madcoda\Youtube\Youtube(array('key' => $pt->config->yt_api));
         $get_videos = $youtube->getVideoInfo($youtube_id);
     if (!empty($get_videos)) {
       ...
       $data_insert['youtube'] = $youtube_id;
...
if ($first == 'import_odysee') {

    $user_id = $_GET['user_id'];
    $odysee = $_GET['odysee'];
   
    try{
       $html = file_get_contents( $odysee );
       preg_match('/"thumbnailUrl\": \"(.*?)"/',$html,$thumb);
       preg_match('/"name\": \"(.*?)"/',$html,$titulo);
      //  preg_match('/"description\": \"(.*?)"/',$html,$description);
       //preg_match('/"duration\":\"(.*?)"/',$html,$duration);
      ...
      $description =getCurlDescription($odysee);      
      ...
      $tags = implode(',', $tags);
       ...
       if ($db->where('odysee', $video_import_id)->getValue(T_VIDEOS, 'count(*)') > 0) {
          $data = array('status' => 400, 'message' => $error_icon . $lang->video_already_exist);
          header('Content-Type: application/json');
          echo json_encode($data);
          exit();
       ...
       $data_insert      = array(
        ...
        $data_insert['odysee'] = $video_import_id;
