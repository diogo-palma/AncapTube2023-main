...
function getCurlDescription($link){
	$description = '';
	try {
		//code...
		$curl = curl_init();
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://ancaptube.com/nodeapp/get-description',
		...
		$description = json_decode($response, true);
		$description = strip_tags(html_entity_decode($description), '<br><p><a><div><span><button>');
...
if (!empty($_POST['link'])) {
	...
	else if (preg_match("#https?://rumble.com/([A-Za-z0-9]+)#s", $link, $match)) {
		$re_data['rumble'] = PT_Secure($match[0]);
		$is_there_video   = true;
		$video_import_id  = $re_data['rumble'];
		$video_type       = 'rumble';
	}

	else if (preg_match("#https?://odysee.com/#s", $link, $match)) {
		$re_data['odysee'] = PT_Secure($match[0]);
		$is_there_video   = true;
		$video_import_id  = $re_data['odysee'];
		$video_type       = 'odysee';
	}

	else if (preg_match("#https?://www.bitchute.com/video/([A-Za-z0-9]+)#s", $link, $match)) {
		$re_data['bitchute'] = PT_Secure($match[0]);
		$is_there_video   = true;
		$video_import_id  = $re_data['bitchute'];
		$video_type       = 'bitchute';
	}
    ...
    if (empty($error)) {
	    ...
	    if (!empty($re_data['youtube'])) {
		...
		else if (!empty($re_data['bitchute'])) {
			$html = file_get_contents( $link );
			preg_match('/<meta property="og:image" content="(.*?)"/', $html, $thumb);
			preg_match('/<meta property="og:title" content="(.*?)"/', $html, $titulo);			
			preg_match('/iframe.*?src=[\'"](.*?)[\'"].*?>/i', $html, $embedUrl);			

			$description =getCurlDescription($link);
			$pattern = '/<span class=\\"more\\">(.*)<\/span>/';
			$description = preg_replace($pattern, '', $description);
			$pattern = '/<span class=\\"less hidden\\">(.*)<\/span>/';
			$description = preg_replace($pattern, '', $description);
			$description = strip_tags($description,  '<br><div>');
			$title = $titulo[1];	
			$thumbnail = $thumb[1];	
			$thumbnail = str_replace('http://', 'https://', $thumbnail);
			$video_import_id = $embedUrl[1];
			
			$tags = preg_replace('/[^[:alpha:]À-ÖØ-öø-ÿ\s]/u', "", $titulo[1]);
			
			$tags = str_replace(' ', ',', $tags);

		}

		else if (!empty($re_data['rumble'])) {
			$html = file_get_contents( $link );
			preg_match('/"thumbnailUrl\":\"(.*?)"/',$html,$thumb);
			preg_match('/"name\":\"(.*?)"/',$html,$titulo);
			//preg_match('/"description\":\"(.*?)"/',$html,$description);
			//preg_match('/"duration\":\"(.*?)"/',$html,$duration);
			preg_match('/"video\":\"(.*?)"/',$html,$embedUrl);
			...
			$video_import_id = $embedUrl[1];
			$title = $titulo[1];
			$tags = preg_replace("/[^a-zA-Z\s]/", "", $titulo[1]);
			$tags = str_replace(' ', ',', $tags);
		...
		else if (!empty($re_data['odysee'])) {
			$html = file_get_contents( $link );
			preg_match('/"thumbnailUrl\": \"(.*?)"/',$html,$thumb);
			preg_match('/"name\": \"(.*?)"/',$html,$titulo);
			//preg_match('/"description\": \"(.*?)"/',$html,$description);
			//preg_match('/"duration\":\"(.*?)"/',$html,$duration);
			preg_match('/"embedUrl\": \"(.*?)"/',$html,$embedUrl);

			preg_match('/"keywords\": \"(.*?)"/',$html,$keywords, PREG_OFFSET_CAPTURE);

			
			$description =getCurlDescription($link);
			
			$description = strip_tags($description,  '<br><a><p><div>');
			 
			$description = preg_replace('/<div.*?>/', '', $description);
			$description = preg_replace('/<\/div>/', '<br><br>', $description);
			$description = preg_replace('/<br.*?>/', '<br>', $description);
			...
			$tags = explode(',', $keywords[1][0]);
			$tags = implode(',', $tags);
			
			$thumbnail = $thumb[1];
			$thumbnail = str_replace('http://', 'https://', $thumbnail);
			$video_import_id = urldecode($embedUrl[1]);
			$title = $titulo[1];			
			//$duration = $duration[1];
